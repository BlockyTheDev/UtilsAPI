...
```java
// Registering my custom command with the name "MyCommand" the permission "command.use" and the error message "You can't use this command" and consuming the event
CommandUtils.registerCommand(new CustomCommand("MyCommand", "command.use", "You can't use this command", e -> {
...
The `IngameConfig` provides an simple way to synchronize your normal config with a minecraft gui or just create a new cusotm config.

```java
// this > Instance of your main class

// Setting up a new IngameConfig without syncing a CustomConfig

// Creating a new IngameConfig with the command /customIngameConfig
IngameConfig config = new IngameConfig("customIngameConfig",
    // Setting the required permission for the command to config.use
    "config.use",
    // Setting the message when someone does not have enough permissions for the command
    "You don't have enough permissions to use this command",
    // Setting the title for the GUI
    "CustomConfig"
    );

// Adding a new entry to the IngameConfig with the name of the item being "Admin"
config.addEntry(new IngameConfigEntry("Admin",
    // Setting the material of the item
    Material.DIAMOND_BLOCK,
    // Setting the key of the config-entry-item
    "Admin Role",
// Setting the value of the config-entry-item
    "Owner",
    // Setting the title of the title when editing the value of the config-entry-item
    "&7Insert new value",
    // Setting the message of the title when editing the value of the config-entry-item
    "&ePlease insert a new value for %n% in chat",
    // Accepting the event
    e -> {
        // Broadcasting a message to the server if the config value was edited, containing various information about the process
        Bukkit.broadcastMessage("You just changed the value for " + e.getKey() + " from " + e.getOldValue() + " to " + e.getNewValue() + ", " + e.getPlayer().getName());
    }));

// Setting up the config which will result in the command being active and working
config.setup();

// Setting up a new IngameConfig which will be synced with the CustomConfig

// Creating a new IngameConfig with the command /customIngameConfig
IngameConfig config = new IngameConfig("customIngameConfig",
    // Setting the required permission for the command to config.use
    // "config.use",
    // Setting our config which will be synced
    customConfig,
    // Setting the message when someone does not have enough permissions for the command
    "You don't have enough permissions to use this command",
    // Setting the title for the GUI
